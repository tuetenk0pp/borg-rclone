...
export BORG_REPO='/mnt/disks/WD_Elements_10B8/borgbackup'

#This is the location you want Rclone to send the BORG_REPO to 
export CLOUDDEST='b2:unraid-backup-borg'

#Speeds up the backup by caching more
#export BORG_FILES_CACHE_TTL=26

#Setting this, so you won't be asked for your repository passphrase:
export BORG_PASSPHRASE='hg9vmernvumv528kgeoeo76a979xpj4ipwwensr5qujrp37xh73vyudiorsjs8zp'
...
borg create                         \
    ...
    --compression lz4               \
    --exclude-caches                \
    --exclude /mnt/user/system/docker/docker.img \
    \
    $BORG_REPO::'{hostname}-{now}'  \
    \
    /mnt/user/appdata               \
    /mnt/user/domains               \
    /mnt/user/factorio-server       \
    /mnt/user/fileDrop              \
    /mnt/user/nextcloud             \
    /mnt/user/paperless-ngx         \
    /mnt/user/system                \
...
#Reset timer
    ...
    rclone sync $BORG_REPO $CLOUDDEST -P --stats 1m -v 2>&1 | tee -a $LOGFILE
    rclonestart=$SECONDS   
...
# All other errors
    ...
    /usr/local/emhttp/webGui/scripts/notify -e "Borg & Rclone" -s "Backup nicht fertig gestellt!" -d "Borg oder Rclone konnte das Backup nicht fertig stellen. Die Logs befinden sich hier: /boot/logs/Borg.txt" -i "warning" -l "/Settings/Userscripts"
fi
exit ${global_exit}